...
def extract_nested_compress(builder_name, file_filename):
    try:
        if file_filename not in os.listdir(f"data/{builder_name}/uploaded/"):
    ...
    flatten_dir(f"data/{builder_name}/uploaded/", f"data/{builder_name}/uploaded/")

    while set(patoolib.ArchiveFormats) & { x.split('.')[-1] for x in os.listdir(f"data/{builder_name}/uploaded/")}:
        for file in os.listdir(f"data/{builder_name}/uploaded/"):
            if file.split('.')[-1] in patoolib.ArchiveFormats and file in os.listdir(f"data/{builder_name}/uploaded/"):
                patoolib.extract_archive(f"data/{builder_name}/uploaded/{file}",
                                         outdir=f"data/{builder_name}/uploaded/")
